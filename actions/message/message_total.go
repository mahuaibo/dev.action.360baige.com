...
func (*MessageTotalAction) Add(args *message.MessageTotal, reply *message.MessageTotal) error {
	...
	if err == nil {
		...
		reply.MessageTempId = args.MessageTempId
		reply.SplitContent = args.SplitContent
...
// 更新 by Id
func (*MessageTotalAction) UpdateById(args *message.MessageTotal, reply *message.MessageTotal) error {
	o := orm.NewOrm()
	o.Using("message")
...
// 1. AddMultiple 增加多个
func (*MessageTotalAction) AddMultiple(args []*message.MessageTotal, reply *batch.BackNumm) error {
	o := orm.NewOrm()
...
func (*MessageTotalAction) UpdateByIds(args *batch.BatchModify, reply *batch.BackNumm) error {
	o := orm.NewOrm()
	o.Using("message")            //查询数据库
...
// 3.查询List （按ID, 按页码）
func (*MessageTotalAction) List(args *paginator.Paginator, reply *paginator.Paginator) error {
	o := orm.NewOrm()
	o.Using("message")            //查询数据库
	qs := o.QueryTable("message") //查询表名
