...
// 更新 by Id
func (*MessageSendAction) UpdateById(args *message.MessageSend, reply *message.MessageSend) error {
	o := orm.NewOrm()
	o.Using("message")
...
// 1. AddMultiple 增加多个
func (*MessageSendAction) AddMultiple(args []*message.MessageSend, reply *batch.BackNumm) error {
	o := orm.NewOrm()
...
func (*MessageSendAction) UpdateByIds(args *batch.BatchModify, reply *batch.BackNumm) error {
	o := orm.NewOrm()
	o.Using("message")            //查询数据库
...
// 3.查询List （按ID, 按页码）
func (*MessageSendAction) List(args *paginator.Paginator, reply *paginator.Paginator) error {
	o := orm.NewOrm()
	o.Using("message")            //查询数据库
	qs := o.QueryTable("message") //查询表名
